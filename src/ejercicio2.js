...
const productosBase = [
  ...
  // (los demás del original no se usan porque la consigna pide 5)
];
...
class Producto {
  ...
  // (no requerido por la consigna, pero útil para probar)
  ...
  // 1) Método estático: valor total del inventario
  static valorTotalInventario(lista) {
    return lista.reduce((acc, pr) => acc + pr.precio * pr.stock, 0);
  }

  // 2) Método estático: aplicar descuento por categoría
  // reglas: { "Periféricos": 10, "Computadores": 5, ... }
